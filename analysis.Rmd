...
runModel<-function(Yobs_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat, parameters.to.save=ParsStage,model.file="model/intercept_interactions.jags",n.thin=1,n.iter=1000,n.burnin=990,n.chains=2,DIC=F)
    )
    return(model)
}
...
chains$intercept %>% filter(parameter %in% "alpha") %>% group_by(Hummingbird,Plant) %>% summarize(n=mean(value)) %>% ggplot(.,aes(fill=n,x=Plant,y=Hummingbird)) + geom_tile() + theme(axis.text.x = element_text(angle=-90)) + labs(fill="P(Interaction)") + scale_fill_gradient2(low="white",mid="blue",high="Red",midpoint=0.5,limits=c(0,1))
```
