...
runModel<-function(Yobs_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,inits = InitStage, parameters.to.save=ParsStage,model.file="model/intercept_interactions_detection.jags",n.thin=2,n.iter=1000,n.burnin=900,n.chains=2,DIC=F)
    )
    return(model)
}
...
getChains<-function(mod){
  ...
  splitpc<-split(pc,pc$parameter)
    ...
    #single index
  splitpc[c("omega","beta")]<-lapply(
    splitpc[c("omega","beta")],function(x){
...
chains$intercept %>% filter(parameter %in% c("omega","beta")) %>% select(-par) %>% spread(parameter,value) %>% group_by(Hummingbird,Draw,chain) %>% do(sim_elevation(alpha=.$omega,beta =.$beta ,ele=unique(indat$ele))) %>% group_by(Hummingbird,ele) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95)) %>% ggplot(.,aes(x=ele)) + geom_ribbon(aes(ymin=lower,ymax=upper)) + geom_line(aes(y=mean),col="Red",linetype="dashed")+ labs(y="Detection Probability") + facet_wrap(~Hummingbird)
```
...
```{r,fig.height=8,fig.width=8}
chains$intercept %>% filter(parameter %in% "alpha") %>% group_by(Hummingbird,Plant) %>% summarize(n=mean(value)) %>% ggplot(.,aes(fill=n,x=Plant,y=Hummingbird)) + geom_tile() + scale_fill_continuous(low="blue",high="red") + theme(axis.text.x = element_text(angle=-90)) + labs(fill="P(Interaction)") + scale_fill_gradient2(low="blue",mid="white",high="Red",midpoint=0.5)
```
