...
dailydat<-interactions %>% group_by(ID,Date,ele,Plant,Hummingbird) %>% summarize(n=n()) %>% select(-n)
```
...
runModel<-function(Yobs_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat, parameters.to.save=ParsStage,model.file="model/poisson_interactions.jags",n.thin=1,n.iter=1000,n.burnin=900,n.chains=2,DIC=F)
    )
    return(model)
}
...
chains$intercept %>% filter(parameter %in% "alpha") %>% group_by(Hummingbird,Plant) %>% summarize(n=mean(exp(value))) %>% ggplot(.,aes(fill=n,x=Plant,y=Hummingbird)) + geom_tile() + theme(axis.text.x = element_text(angle=-90)) + labs(fill="P(Interaction)") + scale_fill_continuous(low="blue",high="Red")
```
...
```{r}
alpha_matrix<-chains$intercept %>% filter(parameter %in% "alpha") %>% group_by(Hummingbird,Plant) %>% summarize(n=mean(exp(value))) %>% acast(Hummingbird~Plant)
