...
runModel<-function(Yobs_dat){
  ...
  #Parameters to track
  ParsStage <- c("alpha","omega","omega_mu","omega_tau")
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat, parameters.to.save=ParsStage,model.file="model/poisson_interactions_detection.jags",n.thin=2,n.iter=100,n.burnin=90,inits = InitStage,n.chains=2,DIC=F)
    )
    return(model)
}
...
#omega
chains$intercept %>% filter(parameter=="omega") %>% ggplot(.,aes(x=Hummingbird,y=value)) + geom_boxplot()  + coord_flip()
```

### I(Interaction)

```{r,fig.height=8,fig.width=8}
chains$intercept %>% filter(parameter %in% "alpha") %>% group_by(Hummingbird,Plant) %>% summarize(n=mean(value)) %>% ggplot(.,aes(fill=exp(n),x=Plant,y=Hummingbird)) + geom_tile() + theme(axis.text.x = element_text(angle=-90)) + labs(fill="P(Interaction)") + scale_fill_gradient2(low="white",mid="blue",high="Red",midpoint=0.5,limits=c(0,1))
```
