...
```{r}
ggplot(transects_zero %>% filter(!n==0),aes(x=Plant,y=ele*1000)) + geom_boxplot() + coord_flip() + labs(y="Elevation (m)") + theme_bw()
ggsave("Figures/ElevationRanges.png",height=4,width=8,dpi=300)
```

## Flowering Data Matrix

```{r}
transects_zero<-transects_zero %>% ungroup() %>% mutate(Index=group_indices(.,Year,Site,Month))

transects_zero %>% ggplot(.) + geom_tile(aes(x=Index,y=Plant,col=Year,fill=as.factor(n>0))) + labs(fill="Flowering",x="Observation")  + theme_bw() + scale_fill_manual(values=c("grey90","black"))
ggsave("Figures/DataMatrix.png",height=3,width=9)
ggsave("Figures/DataMatrix.svg",height=3,width=9)
```
...
null_data<-lapply(1:500,function(x){
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(    
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/occurrence.jags",n.thin=1,n.iter=20000,n.burnin=19500,n.chains=2,DIC=F)
    )
    return(model)
}
...
chains$baseline %>% filter(parameter %in% c("fit","fitnew","fitpred")) %>% ggplot(.,aes(x=value)) + geom_histogram() + facet_wrap(~parameter,ncol=1,scales="free")
```

### Predict flowering rates by elevation
...
```{r}
sample_point<-indat[50,]
sample_point
transects_zero %>% filter(Site==sample_point$Site,Month==sample_point$Month,Plant==sample_point$Plant)
chains$baseline %>% filter(Index==100) %>% head()
```

Probability of observing a flowering record
```{r}
chains$baseline %>% filter(Index==100,parameter=="p") %>% ggplot(.,aes(x=value)) + geom_histogram() 
```

Discrepancy
```{r}
chains$baseline %>% filter(Index==100,parameter=="discrepancy") %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05))
```

Fit by presence/abs
```{r}
chains$baseline %>% filter(parameter=="discrepancy") %>% group_by(Draw,chain,n) %>% summarize(fit=mean(value)) %>% ggplot(.,aes(x=fit,fill=as.factor(n))) + geom_histogram() 
```
...
```{r}
#Source model
source("model/occurrence_month.R")

#print model
writeLines(readLines("model/occurrence_month.R"))

#Run Model
runModel<-function(Yobs_dat,Ynew_dat){
  
  #Parameters to track
  ParsStage <- c("alpha","beta","fit","fitnew","discrepancy","pred_error","fitpred","p","p_new")
  ...
  #MCMC options
    system.time(    
      model<-jags(data=Dat,parameters.to.save=ParsStage,inits=initStage,model.file="model/occurrence_month.jags",n.thin=1,n.iter=5000,n.burnin=4500,n.chains=2,DIC=F)
    )
    return(model)
}

models$occurrence_month<-runModel(Yobs_dat=indat,Ynew_dat=to_predict)
```
...
getChains<-function(mod,indat,to_predict){
  ...
  #single index
  splitpc[c("beta")]<-lapply(
    splitpc[c("beta")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+)]"))[,3]
    pc<-data.frame(x,jPlant=as.numeric(as.character(sv)))  %>% left_join(jagsIndexPlants)
    })
    ...
    sv$jPlant<-as.integer(as.character(sv$jPlant))
  ...
  #Prediction
    splitpc[c("p_new","pred_error")]<-lapply(
    splitpc[c("p_new","pred_error")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+)]"))[,3]
    pc<-data.frame(x,Index=sv)
    pc<-pc %>% inner_join(to_predict)
    })
...
```{r}
chains$occurrence_month<-getChains(models$occurrence_month,indat,to_predict)
```
...
```{r,fig.height=20,fig.width=15}
chains$occurrence_month %>% filter(parameter %in% c("beta")) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~Plant,scales="free")
```

month intercepts
```{r,fig.height=20,fig.width=15}
chains$occurrence_month %>% filter(parameter %in% c("alpha")) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_grid(Month~Plant,scales="free")
```

```{r}
chains$occurrence_month %>% filter(parameter %in% c("fit","fitnew","fitpred")) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free",ncol=1) + labs(col="chain")
```

### Posterior estimates

```{r,fig.height=10,fig.width=10}
chains$occurrence_month %>% filter(parameter %in% c("fit","fitnew","fitpred")) %>% ggplot(.,aes(x=value)) + geom_histogram() + facet_wrap(~parameter,ncol=1,scales="free")
```

Fit by presence/absence

```{r}
chains$occurrence_month %>% filter(parameter=="discrepancy") %>% group_by(Draw,chain,n) %>% summarize(fit=mean(value)) %>% ggplot(.,aes(x=fit,fill=as.factor(n))) + geom_histogram() 
```
...
Confirm everything matches up
```{r}
sample_point<-indat[20,]
sample_point
transects_zero %>% filter(Plant==sample_point$Plant,Site==sample_point$Site,Month==sample_point$Month)
```

Probability of observing a flowering record
```{r}
chains$occurrence_month %>% filter(Index==100,parameter=="p") %>% ggplot(.,aes(x=Draw,y=value, col=as.factor(chain))) + geom_line() 
chains$occurrence_month %>% filter(Index==100,parameter=="p") %>% ggplot(.,aes(x=value)) + geom_histogram() 

```

Discrepancy
```{r}
chains$occurrence_month %>% filter(Index==100,parameter=="discrepancy") %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05))
```

### Predict flowering rates 

```{r}
predicted_rates<-chains$occurrence_month %>% filter(parameter %in% c("p")) %>% group_by(Plant,Month) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
...
ggplot(predicted_rates,aes(x=Month,y=mean,ymin=lower,ymax=upper)) + geom_ribbon(alpha=0.4) + geom_line() + facet_wrap(~Plant) + labs(x="Month",y="p(Flowering)") + scale_x_continuous(breaks=1:12) + theme_bw() + geom_point(data=pplot,aes(col=Predict))
```
...
alpha_estimates<-chains$occurrence_month %>% filter(parameter %in% c("alpha")) %>% group_by(Plant,Draw,Month,chain) %>% select(Draw,chain,parameter,Plant,Month,value) %>% spread(parameter,value)

#merge with beta estimates
predicted_ranges<-chains$occurrence_month %>% filter(parameter %in% c("beta")) %>% group_by(Plant,Draw,chain) %>% select(Draw,chain,parameter,Plant,value) %>% spread(parameter,value) %>% inner_join(alpha_estimates) %>% group_by(Draw,chain,Plant,Month) %>% do(predict_range(.$alpha,.$beta)) %>% group_by(Plant,Month,ele) %>% summarize(mean=mean(p),lower=quantile(p,0.05),upper=quantile(p,0.95))
ggplot(predicted_ranges,aes(x=ele,y=mean,ymin=lower,ymax=upper,col=as.factor(Month)))  + geom_line(aes(group=Month)) + facet_wrap(~Plant,scales="free") + labs(x="Elevation(m)",y="p(Occurrence)")
```
...
writeLines(readLines(paste("model/",mod_name,".R",sep="")))
  ...
  #Parameters to track
  ParsStage <- c("alpha","beta","fit","fitnew","discrepancy","pred_error","lambda_cov","omega","fitpred","p","p_new","gamma","e")
  ...
  #MCMC options
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file=paste("model/",mod_name,".jags",sep=""),n.thin=1,n.iter = 5000,n.burnin=4500,n.chains=2)
    return(model)
}
```
...
getChains<-function(mod,indat,to_predict){
  ...
  splitpc[c("discrepancy","p")]<-lapply(
    splitpc[c("discrepancy","p")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+)]"))[,3]
    pc<-data.frame(x,Index=as.character(sv))
    pc<-pc %>% inner_join(indat)
    })
  ...
  splitpc[c("beta")]<-lapply(
    ...
    splitpc[c("alpha","e")]<-lapply(
    splitpc[c("alpha","e")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+),(\\d+)]"))[,3:4]
    colnames(sv)<-c("jPlant","Month")
    sv$Month<-as.numeric(as.character(sv$Month))
    sv$jPlant<-as.integer(as.character(sv$jPlant))
...
```{r}
models$interaction_attraction<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,M=Dint,mod_name = "occurrence_attraction")
```
...
```{r,fig.height=20,fig.width=15}
chains$interaction_attraction %>% filter(parameter %in% c("beta")) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~Plant,scales="free")
```

month intercepts
```{r,fig.height=20,fig.width=15}
chains$interaction_attraction %>% filter(parameter %in% c("alpha")) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_grid(Month~Plant,scales="free")
```
...
### Posterior estimates
```{r,fig.height=10,fig.width=10}
chains$interaction_attraction %>% filter(!parameter %in% c("discrepancy","p_new","p","prediction","pred_error")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=2,scales="free")
```
...
```{r}
models$interaction_repulsion<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,M=Dint,mod_name = "occurrence_repulsion")
```
...
allchains %>% filter(parameter %in% "e",Plant=="Glossoloma purpureum") %>% group_by(Model,Plant,Month) %>% mutate(value=inv.logit(value)) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95)) %>% ggplot(.,aes(x=Month,y=mean,col=Model)) + geom_pointrange(aes(ymin=lower,ymax=upper)) + labs(x="Month",y="inv.logit(e)") + ggtitle("Effect of association on flowering") 
```
...
```{r}
  #true flowering rate
  true_rate<-transects_zero %>% mutate(Predict=as.factor(Year==2017)) %>%mutate(Predict=recode(Predict,`FALSE`="Training",`TRUE`="Prediction")) %>% group_by(Plant,Month,Predict,Year) %>% summarize(true_rate=sum(n)/n())
  
error_rate<-allchains %>% filter(parameter %in% c("p","p_new")) %>% inner_join(true_rate) %>% mutate(error=abs(value-true_rate)) %>% group_by(Model,Plant,Month,Predict) %>% summarize(mean=mean(error),lower=quantile(error,0.05),upper=quantile(error,0.95))
ggplot(error_rate,aes(x=Month,y=mean,ymin=lower,ymax=upper)) + geom_line(aes(col=Model)) + facet_wrap(~Plant)
ggplot(error_rate,aes(x=Model,y=mean,ymin=lower,ymax=upper,col=Predict)) + geom_pointrange() + coord_flip()
```
...
```{r}
ynewpredict<-allchains %>% filter(parameter %in% "p_new",Model %in% c("interaction_attraction","conditional_month"),Plant=="Besleria solanoides") %>% mutate(Model=recode(Model,baseline_site_month="Conditional Month","interaction_attraction"="Visitor Attraction"))   %>% mutate(Site=recode(Site,`1`="1300m-1500m",`2`="1500m-1700m",`3`="1700m-1900m",`4`="1900m-2100m",`5`="2100m-2300m",`6`="2300m-2500m")) %>%  group_by(Model,Plant,Site,Month) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.)  + geom_ribbon(alpha=0.3,aes(x=Month,y=mean,fill=Model,ymin=lower,ymax=upper))  + geom_line(aes(x=Month,y=mean,col=Model)) + scale_x_continuous(breaks=1:12) + facet_wrap(~Site)
...
Overlay observed
```{r,fig.height=10,fig.width=12}
ynewpredict<-allchains %>% filter(parameter %in% "p",Model %in% c("interaction_attraction","conditional_month")) %>% mutate(Model=recode(Model,conditional_month="Conditional + Month","interaction_attraction"="Visitor Attraction")) %>% group_by(Model,Plant,Month) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.) + geom_ribbon(aes(x=Month,ymin=lower,ymax=upper,fill=Model),alpha=0.2) + geom_line(aes(x=Month,y=mean,col=Model))  + facet_wrap(~Plant) + scale_x_continuous(breaks=1:12)
...
```{r,fig.height=10,fig.width=12}
ynewpredict<-allchains %>% filter(parameter %in% "p_new",Model %in% c("interaction_attraction","conditional_month")) %>% mutate(Model=recode(Model,conditional_month="Conditional + Month","interaction_attraction"="Visitor Attraction")) %>% group_by(Model,Plant,Month) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.) + geom_ribbon(aes(x=Month,ymin=lower,ymax=upper,fill=Model),alpha=0.2) + geom_line(aes(x=Month,y=mean,col=Model))  + facet_wrap(~Plant) + scale_x_continuous(breaks=1:12)
