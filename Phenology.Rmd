...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/normal_julian.jags",n.thin=1,n.iter=10,n.burnin=0,n.chains=2,DIC=F)
    )
    return(model)
}
...
getChains<-function(mod){
  ...
  #ragged index
  splitpc[c("sq","Ynew","prediction")]<-lapply(
    splitpc[c("sq","Ynew","prediction")],function(x){
    #get index
...
ggplot(preds) + geom_density(fill="black",aes(x=value)) + facet_wrap(~Plant,scales="free",ncol=4) + labs(x="Julian Day",y="Density") + geom_density(data=indat,aes(x=julian),alpha=0.5,fill="red") + theme_bw()
```
...
ggplot(preds) + geom_density(fill="black",aes(x=value)) + facet_wrap(~Plant,scales="free",ncol=4) + labs(x="Julian Day",y="Density") + geom_density(data=to_predict,aes(x=julian),alpha=0.5,fill="red") + theme_bw()
```
...
#Run Model
runModel<-function(Yobs_dat,Ynew_dat,D=D){
  
  
  #Parameters to track
  ParsStage <- c("alpha","sigma","fit","fitnew","sq","lambda","e","omega","fitpred","beta","Ynew")
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/normal_attraction.jags",n.thin=1,n.iter=30000,n.burnin=29500,n.chains=2,DIC=F)
    )
    return(model)
}

models$phylogenetic_attraction<-runModel(Yobs_dat=indat,Ynew_dat=to_predict,D=D)
```
...
getChains<-function(mod){
  ...
  #single index
  splitpc[c("sigma","e")]<-lapply(
    splitpc[c("sigma","e")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+)]"))[,3]
    pc<-data.frame(x,jPlant=sv)
    })
...
```{r}
chains$phylogenetic_attraction %>% filter(parameter %in% c("fit","fitnew","lambda","omega","fitpred")) %>%  ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free")
```
...
predict_density<-function(x){
  #grab alphas
  alphas<- x %>% filter(parameter %in% c("alpha")) %>% select(Draw,chain,parameter,value) %>% spread(parameter,value)
  
  preds<-x %>% select(-par) %>% filter(parameter %in% c("sigma","e"))  %>% spread(parameter,value) %>% inner_join(alphas) %>% group_by(Draw,chain,Plant) %>% do(predict_plant(.$alpha,.$e,.$sigma)) %>% group_by(Plant,Day) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
}
...
preds<-predict_density(chains$phylogenetic_attraction)
...
runModel<-function(Yobs_dat,Ynew_dat,D=D){
  ...
  #Parameters to track
  ParsStage <- c("alpha","sigma","fit","fitnew","sq","lambda","e","omega","fitpred")
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/normal_repulsion.jags",n.thin=1,n.iter=30000,n.burnin=29500,n.chains=2,DIC=F)
    )
    return(model)
}

models$phylogenetic_repulsion<-runModel(Yobs_dat=indat,Ynew_dat=to_predict,D=D)
```
...
```{r}
chains$phylogenetic_repulsion %>% filter(par %in% c("fit","fitnew","lambda","omega","fitpred")) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free")
```
...
```{r}
preds<-predict_density(chains$phylogenetic_repulsion)
...
#Run Model
runModel<-function(Yobs_dat,Ynew_dat,D){
  
  
  #Parameters to track
  ParsStage <- c("alpha","sigma","fit","fitnew","sq","lambda","e","fitpred")
...
models$trait_attraction<-runModel(Yobs_dat=indat,Ynew_dat=to_predict,D=Dtraits)
```
...
```{r}
chains$trait_attraction %>% filter(par %in% c("fit","fitnew","lambda","fitpred")) %>%  ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free")
```
...
```{r}
preds<-predict_density(chains$trait_attraction)
...
models$trait_repulsion<-runModel(Yobs_dat=indat,Ynew_dat=to_predict,D=Dtraits)
```
...
```{r}
chains$trait_repulsion %>% filter(par %in% c("fit","fitnew","lambda","omega","fitpred")) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free")
```
...
```{r}
preds<-predict_density(chains$trait_repulsion)
...
### Estimated phenology density

```{r,fig.height=12,fig.width=12}
#need to compute seperately for julian and the other models
  allalphas<- allchains %>% filter(parameter %in% c("alpha")) %>% select(Model,Draw,chain,parameter,value) %>% spread(parameter,value)
  
  correlated_predictions<-allchains %>% select(-par) %>% filter(!Model %in% "julian",parameter %in% c("sigma","e"))  %>% spread(parameter,value) %>% inner_join(allalphas) %>% group_by(Model,Draw,chain,Plant) %>% do(predict_plant(.$alpha,.$e,.$sigma)) %>% group_by(Model,Plant,Day) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
  
ggplot(correlated_predictions)  + geom_ribbon(aes(x=Day,ymin=lower,ymax=upper,fill=Model),alpha=0.5) + facet_wrap(~Plant,scales="free",ncol=3) + labs(x="Julian Day",y="Density") + geom_density(data=indat,aes(x=julian),alpha=0.001,fill="red") + theme_bw()
```
...
ggplot(correlated_predictions)  + geom_ribbon(aes(x=Day,ymin=lower,ymax=upper,fill=Model),alpha=0.5) + facet_wrap(~Plant,scales="free",ncol=3) + labs(x="Julian Day",y="Density") + geom_density(data=transects %>% filter(Year %in% 2017),aes(x=julian),alpha=0.001,fill="red") + theme_bw()
```
