...
writeLines(readLines(paste("model/",mod_name,".R",sep="")))
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file=paste("model/",mod_name,".jags",sep=""),n.thin=1,n.iter=2000,n.burnin=1500,n.chains=2,DIC=F)
    )
    return(model)
}
...
allchains %>% filter(parameter %in% "e",Plant=="Besleria solanoides") %>% group_by(Model,jPlant,Month) %>% mutate(value=value) %>%  summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95)) %>% inner_join(jagsIndexPlants) %>% ggplot(.,aes(y=mean,x=Model,col=as.factor(Month))) + geom_pointrange(aes(ymin=lower,ymax=upper)) + facet_wrap(Month~Plant) + labs(y="Effect of Association on Flowering",x="",col="Site") + coord_flip()
```
