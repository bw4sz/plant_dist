...
```{r,eval=T}
intdata<-as.matrix(read.csv("data/InteractionProbability.csv",row.names = 1))
...
ggplot(reshape2::melt(Dint),aes(x=Var1,y=Var2,fill=value)) + geom_tile() + scale_fill_continuous(low="red",high="blue") + theme(axis.text.x = element_text(angle = -90)) + labs(fill="Visitor Distance") 
```
...
D<-D[as.vector(jagsIndexPlants$Plant),as.vector(jagsIndexPlants$Plant)]
Dtraits<-Dtraits[as.vector(jagsIndexPlants$Plant),as.vector(jagsIndexPlants$Plant)]
...
#Correlation in association matrices
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #Parameters to track
  ParsStage <- c("alpha","fit","fitnew","discrepancy","pred_error","fitpred","p_new","p")
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/threshold_baseline.jags",n.thin=2,n.iter=10000,n.burnin=9500,n.chains=2,DIC=F)
    )
    return(model)
}
...
# Baseline transect model

Equal probability of flowering per elevation

```{r}
#Source model
source("model/threshold_baseline_site.R")

#print model
writeLines(readLines("model/threshold_baseline_site.R"))

#Run Model
runModel<-function(Yobs_dat,Ynew_dat){
  
  #Parameters to track
  ParsStage <- c("alpha","fit","fitnew","discrepancy","pred_error","fitpred","p","p_new")
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/threshold_baseline_site.jags",n.thin=2,n.iter=10000,n.burnin=9500,n.chains=2,DIC=F)
    )
    return(model)
}

models$baseline_site<-runModel(Yobs_dat=indat,Ynew_dat=to_predict)
```
...
```{r}
chains$baseline_site<-getChains(models$baseline_site,indat,to_predict)
```

### Evaluate convergence

```{r,fig.height=12,fig.width=15}
chains$baseline_site %>% filter(parameter %in% c("alpha")) %>%                    ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(Site~Plant,scales="free")
```

```{r}
chains$baseline_site %>% filter(parameter %in% c("fit","fitnew","fitpred")) %>%                    ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free",ncol=1) + labs(col="chain")
```

### Posterior estimates

```{r,fig.height=10,fig.width=10}
chains$baseline_site %>% filter(parameter %in% c("alpha","fit","fitnew","fitpred")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=1,scales="free")
```
...
writeLines(readLines(paste("model/",mod_name,".R",sep="")))
  ...
  #MCMC options
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file=paste("model/",mod_name,".jags",sep=""),n.thin=2,n.iter=10000,n.burnin=9500,n.chains=2,DIC=F)
    return(model)
}

models$phylogenetic_attraction<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,M=D,mod_name = "threshold_attraction")
```
...
#Sample random e to view
chains$phylogenetic_attraction %>% filter(parameter %in% c("e"),jPlant==1) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(Month~Site,scales="free")
```
...
### Posterior estimates
```{r,fig.height=10,fig.width=10}
chains$phylogenetic_attraction %>% filter(!parameter %in% c("discrepancy","p_new","pred_error")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=2,scales="free")
```
...
```{r}
phy_effect<-chains$phylogenetic_attraction %>% filter(parameter %in% c("lambda_cov","omega","gamma")) %>% select(-par) %>% spread(parameter,value) %>% group_by(Draw,chain) %>% do(cor_effect(lambda=.$lambda_cov,D=D,omega=.$omega,gamma=.$gamma)) %>% group_by(Distance) %>% filter(!Distance==0) %>% summarize(mean=mean(Correlation),lower=quantile(Correlation,0.05),upper=quantile(Correlation,0.95))

ggplot(phy_effect,aes(x=Distance,y=mean)) + geom_line() + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.5) + theme_bw() + labs(x="Distance",y="Correlation")
```

## Repulsion

```{r}
models$phylogenetic_repulsion<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,M=D,mod_name = "threshold_repulsion")
```
...
### Posterior estimates
```{r,fig.height=10,fig.width=10}
chains$phylogenetic_repulsion %>% filter(!parameter %in% c("discrepancy","p_new","prediction","pred_error")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=2,scales="free")
```
...
```{r}
phyrepulse_effect<-chains$phylogenetic_repulsion %>% filter(parameter %in% c("lambda_cov","omega","gamma")) %>% select(-par) %>% spread(parameter,value) %>% group_by(Draw,chain) %>% do(cor_effect_repulsion(lambda=.$lambda_cov,D=D,omega=.$omega,gamma=.$gamma)) %>% group_by(Distance) %>% filter(!Distance==0) %>% summarize(mean=mean(Correlation),lower=quantile(Correlation,0.05),upper=quantile(Correlation,0.95))

ggplot(phyrepulse_effect,aes(x=Distance,y=mean)) + geom_line() + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.5) + theme_bw() + labs(x="Distance",y="Correlation")
```
...
```{r}
models$trait_attraction<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,M=Dtraits,mod_name = "threshold_attraction")
```
...
### Posterior estimates
```{r,fig.height=10,fig.width=10}
chains$trait_attraction %>% filter(!parameter %in% c("discrepancy","p_new","prediction","pred_error")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=2,scales="free")
```
...
```{r}
trait_effect<-chains$trait_attraction %>%  filter(parameter %in% c("lambda_cov","omega","gamma")) %>% select(-par) %>% spread(parameter,value) %>% group_by(Draw,chain) %>% do(cor_effect(lambda=.$lambda_cov,D=Dtraits,omega=.$omega,gamma=.$gamma)) %>% group_by(Distance) %>% filter(!Distance==0) %>% summarize(mean=mean(Correlation),lower=quantile(Correlation,0.05),upper=quantile(Correlation,0.95))
ggplot(trait_effect,aes(x=Distance,y=mean)) + geom_line() + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.5) + theme_bw() + labs(x="Distance",y="Correlation")
```

## Repulsion

```{r}
models$trait_repulsion<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,M=Dtraits,mod_name = "threshold_repulsion")
```
...
### Posterior estimates
```{r,fig.height=10,fig.width=10}
chains$trait_repulsion %>% filter(!parameter %in% c("discrepancy","p_new","prediction","pred_error")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=2,scales="free")
```
...
```
```{r}
traitrepulse_effect<-chains$trait_repulsion %>% filter(parameter %in% c("lambda_cov","omega","gamma")) %>% select(-par) %>% spread(parameter,value) %>% group_by(Draw,chain) %>% do(cor_effect_repulsion(lambda=.$lambda_cov,D=Dtraits,omega=.$omega,gamma=.$gamma)) %>% group_by(Distance) %>% filter(!Distance==0) %>% summarize(mean=mean(Correlation),lower=quantile(Correlation,0.05),upper=quantile(Correlation,0.95))

ggplot(traitrepulse_effect,aes(x=Distance,y=mean)) + geom_line() + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.5) + theme_bw() + labs(x="Distance",y="Correlation")
```
...
ggplot(int_effect,aes(x=Distance,y=mean)) + geom_line() + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.5) + theme_bw() + labs(x="Distance",y="Correlation")
```
...
```{r,fig.height=40,fig.width=12}
allchains %>% filter(parameter %in% "p_new") %>% group_by(Model,jPlant,Site,Month) %>% summarize(p=mean(value))  %>% inner_join(jagsIndexPlants) %>% ggplot(.,aes(x=as.factor(Month),y=Plant)) + geom_tile(aes(fill=p)) + facet_wrap(Site~Model,ncol=4) + scale_fill_continuous(low="blue",high="red") + labs(fill="p(Flowering)") 
```

```{r,fig.height=40,fig.width=12}
 allchains %>% filter(parameter %in% "pred_error") %>% group_by(Model,jPlant,Site,Month) %>% summarize(mean=mean(value)) %>% inner_join(jagsIndexPlants) %>% ggplot(.,aes(x=as.factor(Month),y=Plant)) + geom_tile(aes(fill=mean)) + facet_wrap(Site~Model,ncol=4) + scale_fill_continuous(low="blue",high="red") + labs(fill="Prediction Discrepancy") 
```
...
```{r}
phy_effect$Model<-"phylogenetic_attraction"
phyrepulse_effect$Model<-"phylogenetic_repulsion"
trait_effect$Model<-"trait_attraction"
traitrepulse_effect$Model<-"trait_tepulsion"
intrepulse_effect$Model<-"interaction_repulsion"
int_effect$Model<-"interaction_attraction"
...
```{r}
Ynewtable<-allchains %>% filter(parameter %in% "p_new") %>% mutate(value=value) %>%  group_by(Model,Plant,Month,Site) %>% summarize(n=mean(value))  

pdat<-indat %>% group_by(Plant,Month) %>% filter(!Year==2017)%>% summarize(Observed=sum(n)/n()) %>% inner_join(Ynewtable) %>% mutate(Month=month.abb[as.numeric(Month)])
...
ggplot(pdat,aes(x=Observed,y=n,col=Model)) + geom_point() + geom_abline() + labs(x="Observed flowering [0-1]",y="Predicted Flowering [0-1]") + facet_wrap(~Month) + coord_equal(xlim = c(0,0.75),ylim=c(0,0.75)) 
```

```{r,fig.height=30,fig.width=12}
allchains %>% filter(parameter %in% "p_new",Model %in% c("trait_repulsion")) %>% group_by(Model,Plant,Month,Site) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.) + geom_ribbon(aes(x=as.factor(Month),ymin=lower,ymax=upper,fill=Model,group=Model),alpha=0.6)  + facet_wrap(Site~Plant,ncol=3)

pplot<-transects_zero %>% mutate(Predict=as.factor(Year==2017)) %>%mutate(Predict=recode(Predict,`FALSE`="Training",`TRUE`="Prediction")) %>% group_by(Plant,Site,Month,Predict) %>% summarize(p=sum(n)/n())

allchains %>% filter(parameter %in% "p_new")  %>% group_by(Model,Plant,Month) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.) + geom_line(aes(x=as.factor(Month),y=mean,col=Model,group=Model))+ facet_wrap(~Plant)  + geom_point(data=pplot,aes(y=p,shape=Predict,x=Month))+ facet_wrap(~Plant,scales="free",ncol=2) + labs(y="Proportion of data in flower",x="Month") + theme_bw() 

allchains %>% filter(parameter %in% "p_new",Model %in% c("trait_repulsion","baseline_site")) %>% group_by(Model,Plant,Month) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.) + geom_line(aes(x=as.factor(Month),y=mean,col=Model,group=Model))+ facet_wrap(~Plant,ncol=2)  + geom_point(data=pplot,aes(y=p,shape=Predict,x=Month))+ facet_wrap(~Plant,scales="free") + labs(y="Proportion of data in flower",x="Month") + theme_bw() 
```
...
transects_zero %>% mutate(Predict=as.factor(Year==2017)) %>% filter(Plant %in% c("Besleria solanoides")) %>%mutate(Predict=recode(Predict,`FALSE`="Training",`TRUE`="Prediction")) %>% group_by(Plant,Month,Site,Predict) %>% summarize(p=sum(n)/n()) %>% ggplot(aes(y=p,col=Predict,x=as.factor(Month))) + geom_point(size=2) + geom_line(aes(group=Predict)) + facet_wrap(Site~Plant,ncol = 2) + labs(y="Proportion of data in flower",x="Month") + theme_bw()

allchains %>% filter(parameter %in% "p_new",Model %in% c("baseline","baseline_site"),Plant %in% c("Besleria solanoides")) %>% group_by(Model,Plant,Month,Site) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.) + geom_line(aes(x=Month,y=mean,col=Model))+ scale_x_continuous(breaks = 1:12) + geom_point(data=pplot[pplot$Plant=="Besleria solanoides",],aes(y=p,shape=Predict,x=Month))+ facet_wrap(Site~Plant,ncol=2) + labs(y="Proportion of data in flower",x="Month") + theme_bw() 

allchains %>% filter(parameter %in% "p_new",Model %in% c("baseline","baseline_site","trait_repulsion","trait_attraction","interaction_repulsion"),Plant %in% c("Besleria solanoides")) %>% group_by(Model,Plant,Month,Site) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.) + geom_line(aes(x=Month,y=mean,col=Model))+ scale_x_continuous(breaks = 1:12) + geom_point(data=pplot[pplot$Plant=="Besleria solanoides",],aes(y=p,shape=Predict,x=Month))+ facet_wrap(Site~Plant) + labs(y="Proportion of data in flower",x="Month") + theme_bw() 

```

```{r,fig.height=10,fig.width=12}
ynewpredict<-allchains %>% filter(parameter %in% "p_new") %>% group_by(Model,Plant,Month) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.) + geom_ribbon(aes(x=Month,ymin=lower,ymax=upper,fill=Model,group=Model),alpha=0.6)  + facet_wrap(~Plant) + scale_x_continuous(breaks=1:12)

pplot<-transects_zero %>% mutate(Predict=as.factor(Year==2017)) %>%mutate(Predict=recode(Predict,`FALSE`="Training",`TRUE`="Prediction")) %>% group_by(Plant,Month,Predict) %>% summarize(p=sum(n)/n())

ynewpredict + geom_point(data=pplot,aes(y=p,col=Predict,x=Month))+ facet_wrap(~Plant,scales="free",ncol=2) + labs(y="Proportion of data in flower",x="Month") + theme_bw() 
```
...
Grab an example
Drymonia teuscheri v Drymonia collegarum

```{r}

observed<-indat %>% filter(Plant %in% c("Drymonia teuscheri","Drymonia collegarum")) %>% group_by(Plant,Site) %>% summarize(p=sum(n)/n()) 

observed %>% ggplot(.,aes(x=Site,y=p,col=Plant)) + geom_point(size=4) + ggtitle("Observed")
observed %>% spread(Plant,p) %>% ggplot(.,aes(x=`Drymonia teuscheri`,y=`Drymonia collegarum`)) + geom_point(size=4) + ggtitle("Observed") + coord_equal()

to_predict %>% filter(Plant %in% c("Drymonia teuscheri","Drymonia collegarum")) %>% group_by(Plant,Site) %>% summarize(p=sum(n)/n()) %>% ggplot(.,aes(x=Site,y=p,col=Plant)) + geom_point(size=4) + ggtitle("Prediction")

model_names<-unique(allchains$Model)

for(x in model_names){
  allchains %>% filter(parameter %in% "p",Model==x) %>% filter(Plant %in% c("Drymonia teuscheri","Drymonia collegarum"))  %>% ggplot(.) + geom_density(aes(x=value),fill="black") + facet_wrap(~Site) + geom_vline(data=observed,aes(xintercept=p,col=Plant),size=2,alpha=.5) + ggtitle(x)
}

    allchains %>% filter(parameter %in% "p") %>% filter(Plant %in% c("Drymonia teuscheri","Drymonia collegarum")) %>%select(Model,Draw,chain,Plant,Site,value) %>%  group_by(Model,Draw,chain,Plant,Site) %>% summarize(value=mean(value)) %>% spread(Plant,value) %>% ggplot(.,aes(x=`Drymonia teuscheri`,y=`Drymonia collegarum`)) + geom_point() + coord_equal() + facet_grid(Site~Model)

  allchains %>% filter(parameter %in% "discrepancy") %>% filter(Plant %in% c("Drymonia teuscheri","Drymonia collegarum"))  %>% ggplot(.) + geom_density(aes(x=value,fill=Model))
  
    allchains %>% filter(parameter %in% "discrepancy") %>% filter(Plant %in% c("Drymonia teuscheri","Drymonia collegarum")) %>% group_by(Model,Plant) %>% summarize(mean=mean(value)) %>% arrange(Plant,mean)
```
...
```{r}
allchains %>% filter(parameter %in% "e") %>% mutate(value=value) %>% group_by(Model,Plant,Site) %>% summarize(p=mean(value)) %>% spread(Plant,p) %>% select(`Drymonia teuscheri`,`Drymonia collegarum`) %>% ggplot(.,aes(x=`Drymonia teuscheri`,`Drymonia collegarum`)) + geom_point() + facet_wrap(~Model) + stat_smooth(method="lm") + coord_equal()
```

inv.logit E
```{r}
allchains %>% filter(parameter %in% "e") %>% mutate(value=inv.logit(value)) %>% group_by(Model,Plant,Site) %>% summarize(p=mean(value)) %>% spread(Plant,p) %>% select(`Drymonia teuscheri`,`Drymonia collegarum`) %>% ggplot(.,aes(x=`Drymonia teuscheri`,`Drymonia collegarum`)) + geom_point() + facet_wrap(~Model) + stat_smooth(method="lm") + coord_equal()
```

```{r}
allchains %>% filter(parameter %in% "e") %>% group_by(Model,Plant,Site) %>% summarize(p=mean(value)) %>% spread(Plant,p) %>% ungroup() %>% select(-Site) %>% group_by(Model) %>% do(cormat(.))%>% filter(Var1=="Drymonia teuscheri",Var2=="Drymonia collegarum") %>% mutate(Correlation_E=value) %>% select(-value)

allchains %>% filter(parameter %in% "p") %>% group_by(Model,Plant,Site) %>% summarize(p=mean(value)) %>% spread(Plant,p) %>% ungroup() %>% select(-Site) %>% group_by(Model) %>% do(cormat(.))%>% filter(Var1=="Drymonia teuscheri",Var2=="Drymonia collegarum") %>% mutate(Correlation_P=value) %>% select(-value)

allchains %>% filter(parameter %in% "p_new") %>% group_by(Model,Plant,Site) %>% summarize(p=mean(value)) %>% spread(Plant,p) %>% ungroup() %>% select(-Site) %>% group_by(Model) %>% do(cormat(.))%>% filter(Var1=="Drymonia teuscheri",Var2=="Drymonia collegarum") %>% mutate(Correlation_Ynew=value) %>% select(-value)
```
...
models_to_run<-c("phylogenetic_attraction","trait_attraction","interaction_attraction")
#which distance matrix to use for each
D_to_use<-list(D,Dtraits,Dint)

for(x in 1:length(models_to_run)){
  result<-allchains %>% filter(parameter %in% c("omega","gamma","lambda_cov"),Model %in% models_to_run[x]) %>% select(-par) %>% spread(parameter,value) %>% group_by(Draw,chain)  %>% do(tauC_attraction(omega=.$omega,gamma=.$gamma,I=I,D=D_to_use[[x]],lambda_cov=.$lambda_cov)) %>% group_by(Var1,Var2) %>% filter(!is.na(value)) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
    result$Model<-models_to_run[x]
  cor_matrices[[x]]<-result
}

models_to_run<-c("phylogenetic_repulsion","trait_repulsion","interaction_repulsion")

for(x in 1:length(models_to_run)){
  result<-allchains %>% filter(parameter %in% c("omega","gamma","lambda_cov"),Model %in% models_to_run[x]) %>% select(-par) %>% spread(parameter,value) %>% group_by(Draw,chain)  %>% do(tauC_repulsion(omega=.$omega,gamma=.$gamma,I=I,D=D_to_use[[x]],lambda_cov=.$lambda_cov)) %>% group_by(Var1,Var2) %>% filter(!is.na(value)) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
  result$Model<-models_to_run[x]
  cor_matrices[[x+3]]<-result
}

result<-bind_rows(cor_matrices)

result %>% mutate(Model=recode(Model,trait_repulsion="Trait Repulsion",trait_attraction = "Trait Attraction",phylogenetic_repulsion = "Phylogenetic Repulsion",phylogenetic_attraction ="Phylogenetic Attraction",interaction_repulsion ="Visitor Repulsion",interaction_attraction="Visitor Attraction",baseline="Baseline",baseline_site = "Baseline by Site")) %>% ggplot(.,aes(x=Var1,y=Var2,fill=mean)) + geom_tile() + scale_fill_gradient2(low="red",mid="white",high="blue") + labs(x="",y="",fill="Mean Correlation") + theme(axis.text.x = element_text(angle=-90)) + facet_wrap(~Model,ncol=2) 
ggsave("Figures/CorrelationMatrix.png",height=9,width=9)
```
...
est_effect %>% mutate(Model=recode(Model,trait_repulsion="Trait Repulsion",trait_attraction = "Trait Attraction",phylogenetic_repulsion = "Phylogenetic Repulsion",phylogenetic_attraction ="Phylogenetic Attraction",interaction_repulsion ="Visitor Repulsion",interaction_attraction="Visitor Attraction",baseline="Baseline")) %>% ggplot(.,aes(x=Distance,y=mean)) + geom_line() + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.5) + theme_bw() + labs(x="Distance",y="Correlation") + facet_wrap(~Model,scales="free",ncol=2)
ggsave("Figures/Decay.png",height=5,width=4)
```
...
ggsave("Figures/Fit.png",height=3,width=8)
```
