...
writeLines(readLines(paste("model/",mod_name,".R",sep="")))
  ...
  #MCMC options
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file=paste("model/",mod_name,".jags",sep=""),n.thin=2,n.iter=50000,n.burnin=49500,n.chains=2,DIC=F)
    return(model)
}
...
```{r}
allchains %>% filter(parameter %in% "pred_error") %>% group_by(Draw,chain,Model,Plant) %>% summarize(value=sum(value)/n()) %>% group_by(Model,Plant) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95)) %>% arrange(mean) %>% ggplot(.,aes(x=Plant,y=mean,col=Model)) + geom_pointrange(aes(ymin=lower,ymax=upper),alpha=0.75,position=position_dodge(width=0.4)) + coord_flip() + labs(y="Prediction Error") + theme_bw()
```
...
allchains %>% filter(parameter %in% "Ynew") %>% group_by(Model,Plant,Site) %>% summarize(p=sum(value)/n()) %>% spread(Plant,p) %>% ungroup() %>% select(-Site) %>% group_by(Model) %>% do(cormat(.))%>% filter(Var1=="Drymonia teuscheri",Var2=="Drymonia collegarum") %>% mutate(Correlation_Ynew=value) %>% select(-value)
```
...
```{r}
transects_zero %>% mutate(Predict=as.factor(Year==2017)) %>%mutate(Predict=recode(Predict,`FALSE`="Training",`TRUE`="Prediction")) %>% group_by(Plant,Month,Predict) %>% summarize(p=sum(n)/n()) %>% ggplot(aes(y=p,col=Predict,x=as.factor(Month))) + geom_point(size=2) + geom_line(aes(group=Predict)) + facet_wrap(~Plant,ncol = 4) + labs(y="Proportion of data in flower",x="Month") + theme_bw()
...
```{r}
svg("Figures/Phylo.svg")
plot(phy)
dev.off()

jpg("Figures/Phylo.png",dpi=400)
plot(phy)
dev.off()
```
