...
#smooth data
indat<-indat %>% group_by(Plant,jPlant,Site,Month,Year) %>% summarize(n=max(n),ele=mean(ele))
...
```{r}
#Source model
source("model/threshold_baseline_elev.R")

#print model
writeLines(readLines("model/threshold_baseline_elev.R"))
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #Inits
  InitStage<-function(){
    z=rep(1,length(Yobs_dat$n))
    z_new=rep(1,length(Ynew_dat$n))
    list(z=z,z_new=z_new)
  }
  
  #Parameters to track
  ParsStage <- c("alpha","alpha_flowering","beta","fit","fitnew","discrepancy","pred_error","fitpred","Ynew")
  ...
  #Jags Data
    ...
    Ypred=Ynew_dat$n,
    Plant_new=Ynew_dat$jPlant,
    ele_new=Ynew_dat$ele)
    
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="model/threshold_baseline_elev.jags",n.thin=2,n.iter=50000,n.burnin=49500,n.chains=2,DIC=F)
    )
    return(model)
}
...
getChains<-function(mod,indat,to_predict){
  ...
  #single index
  splitpc[c("alpha","beta","alpha_flowering")]<-lapply(
    splitpc[c("alpha","beta","alpha_flowering")],function(x){
...
writeLines(readLines(paste("model/",mod_name,".R",sep="")))
  ...
  #Parameters to track
  ParsStage <- c("alpha","beta","fit","fitnew","discrepancy","pred_error","lambda_cov","e","omega","fitpred","Ynew","gamma")
  ...
  #Jags Data
    ...
    Ypred=Ynew_dat$n,
    Plant_new=Ynew_dat$jPlant,
    Site_new=Ynew_dat$Site,
    Month_new=Ynew_dat$Month,
    ele_new=Ynew_dat$ele)
    
    
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file=paste("model/",mod_name,".jags",sep=""),n.thin=2,n.iter=50000,n.burnin=49500,n.chains=2,DIC=F)
    )
    return(model)
}

models$phylogenetic_attraction<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,D=D,mod_name = "threshold_attraction_elev")
```
...
```{r}
models$phylogenetic_repulsion<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,D=D,mod_name = "threshold_repulsion_elev")
```
...
```{r}
models$trait_attraction<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,D=Dtraits,mod_name = "threshold_attraction_elev")
```
...
```{r}
models$trait_repulsion<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,D=Dtraits,mod_name = "threshold_repulsion_elev")
```
...
```{r}
models$interaction_attraction<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,D=Dint,mod_name = "threshold_repulsion_elev")
```
...
```{r}
models$interaction_repulsion<-runModel_autocorrelation(Yobs_dat=indat,Ynew_dat=to_predict,D=Dint,mod_name = "threshold_repulsion_elev")
```
...
sim_elev<-function(alpha,beta,ele){
  p<-inv.logit(alpha + beta * ele)
  return(data.frame(alpha,beta,ele,p))
}

#The baseline model is not logit transformed intercept
allchains %>% filter(parameter %in% c("alpha","beta")) %>% group_by(Model,Draw,chain,Plant) %>% select(-par) %>% spread(parameter,value) %>% do(sim_elev(.$alpha,.$beta,unique(indat$ele))) %>% group_by(Model,Plant,ele) %>%  summarize(mean=mean(p),lower=quantile(p,0.05),upper=quantile(p,0.95)) %>% ggplot(.,aes(fill=Model,x=ele,y=mean)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.5) + facet_wrap(~Plant,scales="free")
```
