...
runModel<-function(Yobs_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/julian.jags",n.thin=1,n.iter=20000,n.burnin=19500,n.chains=2,DIC=F)
    )
    return(model)
}
...
```{r}
#Source model
source("model/julian_phylogeny2.R")

#print model
writeLines(readLines("model/julian_phylogeny2.R"))
...
runModel<-function(Yobs_dat,I=I,vcov=vcov){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/julian_phylogeny2.jags",n.thin=1,n.iter=1000,n.burnin=900,n.chains=2,DIC=F)
    )
    return(model)
}
...
runModel<-function(Yobs_dat,I=I,vcov=vcov){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/julian_phylogeny2.jags",n.thin=1,n.iter=20000,n.burnin=19500,n.chains=2,DIC=F)
    )
    return(model)
}
...
#label variable
v<-data.frame(Model=c("julian","phylogeny","phylogeny_repulsion"),variable=c("Julian","Phylogeny","Phylogeny"))
