...
ggbiplot(prcomp(traits[,-1]),labels = rownames(traits))
ggsave("Figures/Trait_biplot_label.svg",height=6,width=6)
ggbiplot(prcomp(traits[,-1]))
ggsave("Figures/Trait_biplotl.svg",height=6,width=6)
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/threshold_baseline.jags",n.thin=2,n.iter=5000,n.burnin=4500,n.chains=2,DIC=F)
    )
    return(model)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/threshold_baseline_site.jags",n.thin=2,n.iter=5000,n.burnin=4500,n.chains=2,DIC=F)
    )
    return(model)
}
...
writeLines(readLines(paste("model/",mod_name,".R",sep="")))
  ...
  #MCMC options
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file=paste("model/",mod_name,".jags",sep=""),n.thin=4,n.iter=60000,n.burnin=59000,n.chains=2,DIC=F)
    return(model)
}
...
result %>% mutate(Model=recode(Model,trait_repulsion="Trait Repulsion",trait_attraction = "Trait Attraction",phylogenetic_repulsion = "Phylogenetic Repulsion",phylogenetic_attraction ="Phylogenetic Attraction",interaction_repulsion ="Visitor Repulsion",interaction_attraction="Visitor Attraction",baseline="Baseline",baseline_site = "Baseline by Site")) %>% filter(Model %in% c("Visitor Attraction","Visitor Repulsion")) %>%  ggplot(.,aes(x=Var1,y=Var2,fill=mean)) + geom_tile() + scale_fill_gradient2(low="red",mid="white",high="blue") + labs(x="",y="",fill="Mean Correlation") + theme(axis.text.x = element_text(angle=-90)) + facet_wrap(~Model,ncol=2) 
ggsave("Figures/CorrelationMatrix.png",height=5,width=9)
```
...
ggsave("Figures/Decay.png",height=5,width=8)
```
