...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/threshold_baseline.jags",n.thin=2,n.iter=20000,n.burnin=19000,n.chains=2,DIC=F)
    )
    return(model)
}
...
writeLines(readLines(paste("model/",mod_name,".R",sep="")))
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file=paste("model/",mod_name,".jags",sep=""),n.thin=2,n.iter=20000,n.burnin=19000,n.chains=2,DIC=F)
    )
    return(model)
}
