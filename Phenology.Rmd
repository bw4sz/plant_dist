...
sumf<-transects %>% group_by(Plant,Site,Month,Year) %>% dplyr::summarize(n=sum(Flowers),ele=mean(ele)) %>% mutate(Year=as.factor(Year))%>% group_by(Plant,Year) %>% filter(n >sum(n)*0.05) %>% mutate(n=(n>0)*1)
...
allchains %>% filter(parameter %in% "p_new",Model %in% c("baseline","baseline_site","interaction_attraction"),Plant %in% c("Besleria solanoides")) %>% group_by(Model,Plant,Month,Site) %>% summarize(mean=mean(value),upper=quantile(value,0.95),lower=quantile(value,0.05)) %>% ggplot(.) + geom_line(aes(x=Month,y=mean,col=Model))+ scale_x_continuous(breaks = 1:12) + geom_point(data=pplot[pplot$Plant=="Besleria solanoides",],aes(y=p,shape=Predict,x=Month))+ facet_wrap(Site~Plant,ncol=2) + labs(y="Proportion of data in flower",x="Month") + theme_bw() 
```
