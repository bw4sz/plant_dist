...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/Poisson_baseline.jags",n.thin=1,n.iter=20000,n.burnin=19500,n.chains=2,DIC=F)
    )
    return(model)
}
...
runModel_attraction<-function(Yobs_dat,Ynew_dat,D=D){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/Poisson_attraction.jags",n.thin=1,n.iter=20000,n.burnin=19500,n.chains=2,DIC=F)
    )
    return(model)
}
...
runModel_repulsion<-function(Yobs_dat,Ynew_dat,D=D){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/Poisson_repulsion.jags",n.thin=1,n.iter=20000,n.burnin=19500,n.chains=2,DIC=F)
    )
    return(model)
}
...
```{r}
models$interaction_attraction<-runModel_attraction(Yobs_dat=indat,Ynew_dat=to_predict,D=Dint)
```
...
```{r}
chains$interaction_attraction<-getChains(models$interaction_attraction,indat,to_predict) %>%  left_join(jagsIndexPlants)
```
...
```{r}
models$interaction_repulsion<-runModel_repulsion(Yobs_dat=indat,Ynew_dat=to_predict,D=Dint)
```

#Get Chains

```{r}
chains$interaction_repulsion<-getChains(models$interaction_repulsion,indat,to_predict) %>%  left_join(jagsIndexPlants)
```
...
```{r}
allchains %>% dplyr::filter(parameter=="Ynew",Model=="phylogenetic_attraction") %>% dplyr::group_by(Plant,Model,Index)  %>% dplyr::summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95)) %>% inner_join(indat) %>%  ggplot(.) + geom_point(aes(x=n,y=lower,col=Model),alpha=0.5) + facet_wrap(~Plant,scales="free") + geom_abline()
```
