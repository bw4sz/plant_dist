...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/normal_julian.jags",n.thin=1,n.iter=50000,n.burnin=49500,n.chains=2,DIC=F)
    )
    return(model)
}
...
runModel_attraction<-function(Yobs_dat,Ynew_dat,D=D){
  ...
  #Parameters to track
  ParsStage <- c("alpha","sigma","fit","fitnew","sq","lambda","e","omega","fitpred","beta","Ynew","prediction","gamma")
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/normal_attraction.jags",n.thin=1,n.iter=50000,n.burnin=49500,n.chains=2,DIC=F)
    )
    return(model)
}
...
```{r}
chains$phylogenetic_attraction %>% filter(parameter %in% c("fit","fitnew","lambda","omega","fitpred","gamma")) %>%  ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free",ncol=2)
```
...
runModel_repulsion<-function(Yobs_dat,Ynew_dat,D=D){
  ...
  #Parameters to track
  ParsStage <- c("alpha","beta","sigma","fit","fitnew","sq","lambda","e","omega","fitpred","Ynew","prediction","gamma")
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/normal_repulsion.jags",n.thin=1,n.iter=50000,n.burnin=49500,n.chains=2,DIC=F)
    )
    return(model)
}
...
```{r}
chains$phylogenetic_repulsion %>% filter(par %in% c("fit","fitnew","lambda","omega","fitpred","gamma")) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free",ncol=2)
```
...
```{r}
chains$trait_attraction %>% filter(par %in% c("fit","fitnew","lambda","fitpred","omega","gamma")) %>%  ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free",ncol=2)
```
...
```{r}
chains$trait_repulsion %>% filter(par %in% c("fit","fitnew","lambda","omega","fitpred","gamma")) %>% ggplot(.,aes(x=Draw,col=as.factor(chain),y=value)) + geom_line() + facet_wrap(~parameter,scales="free",ncol=2)
```
...
## Sigma: The variance in mean flowering date

```{r,fig.height=12,fig.width=10}
allpars %>% filter(parameter %in% "sigma") %>% ggplot(.,aes(x=Model,y=mean)) + geom_pointrange(aes(ymin=lower,ymax=upper)) + facet_wrap(~Plant,scales="free",ncol=3) + coord_flip() + labs(y="1 sd of mean flowering date")
```
...
## Gamma: The variance of the effect of autocorrelation on mean flowering date

```{r,fig.height=12,fig.width=10}
allpars %>% filter(parameter %in% "gamma") %>% ggplot(.,aes(x=Model,y=mean)) + geom_pointrange(aes(ymin=lower,ymax=upper)) + facet_wrap(~Plant,scales="free",ncol=3) + coord_flip() + labs(y="Gamma")
```
...
ggplot(preds) + geom_density(aes(x=value,fill=Model),alpha=0.2) + facet_wrap(~Plant,scales="free",ncol=3) + labs(x="Julian Day",y="Density") + geom_density(data=indat,aes(x=julian),size=1.2) + theme_bw()
```
