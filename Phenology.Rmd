...
#As a test lets randomize the matrix?
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/threshold_baseline.jags",n.thin=2,n.iter=4000,n.burnin=3500,n.chains=2,DIC=F)
    )
    return(model)
}
...
writeLines(readLines(paste("model/",mod_name,".R",sep="")))
  ...
  #MCMC options
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file=paste("model/",mod_name,".jags",sep=""),n.thin=2,n.iter=4000,n.burnin=3500,n.chains=2,DIC=F)
    return(model)
}
...
models<-unique(allchains$Model)

for(x in models){
  allchains %>% filter(parameter %in% "p",Model==x) %>% filter(Plant %in% c("Drymonia teuscheri","Drymonia collegarum"))  %>% ggplot(.) + geom_density(aes(x=value)) + facet_wrap(~Site) + geom_vline(data=observed,aes(xintercept=p,col=Plant),size=2,alpha=.5) + ggtitle(x)
