...
runModel<-function(Yobs_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/normal_julian.jags",n.thin=1,n.iter=30000,n.burnin=29500,n.chains=2,DIC=F)
    )
    return(model)
}
...
### Posterior estimates
```{r,fig.height=10,fig.width=10}
chains$phylogenetic_attraction %>% filter(!par %in% c("sq")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=1,scales="free")
```
...
### Posterior estimates
```{r,fig.height=10,fig.width=10}
chains$phylogenetic_repulsion %>% filter(!par %in% c("sq")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=1,scales="free")
```
...
phyrepulse_effect<-chains$phylogenetic_repulsion %>% filter(parameter=="lambda") %>% group_by(Draw,chain) %>% do(effect(.$value,D)) %>% group_by(Distance) %>% filter(!Distance==0) %>% summarize(mean=mean(Covariance),lower=quantile(Covariance,0.05),upper=quantile(Covariance,0.95))
...
### Posterior estimates
```{r,fig.height=10,fig.width=10}
chains$trait_attraction %>% filter(!par %in% c("sq")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=1,scales="free")
```
...
### Posterior estimates
```{r,fig.height=10,fig.width=10}
chains$trait_repulsion %>% filter(!par %in% c("sq")) %>% ggplot(.,aes(x=value,fill=Plant)) + geom_histogram() + facet_wrap(~parameter,ncol=1,scales="free")
```
...
```{r,eval=F}
allchains %>% filter(!parameter %in% c("sq")) %>% group_by(Model,par) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95)) %>% data.frame() %>% kable() %>% kable_styling()
```
...
ggplot(correlated_predictions)  + geom_ribbon(aes(x=Day,ymin=lower,ymax=upper,fill=Model),alpha=0.2) + geom_line(aes(x=Day,y=mean)) + facet_wrap(~Plant,scales="free") + labs(x="Julian Day",y="Density") + geom_density(data=indat,aes(x=julian),alpha=0.1,fill="red") + theme_bw()
```
...
ab<-data.frame(x=seq(ymin,ymax,0.1),y=seq(ymin,ymax,0.1))

ggplot(data=fitstat,aes(x=fit,y=fitnew)) + geom_point(aes(col=Model)) + theme_bw() + labs(x="Discrepancy of observed data",y="Discrepancy of replicated data",col="Model")  + ggtitle("Posterior predictive check") + geom_line(data=ab,aes(x=x,y=y),group=1,linetype="dashed") + coord_fixed()
