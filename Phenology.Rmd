...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file="model/threshold_baseline.jags",n.thin=2,n.iter=100,n.burnin=90,n.chains=2,DIC=F)
    )
    return(model)
}
...
writeLines(readLines(paste("model/",mod_name,".R",sep="")))
  ...
  #MCMC options
    system.time(
      model<-jags(data=Dat,parameters.to.save=ParsStage,model.file=paste("model/",mod_name,".jags",sep=""),n.thin=2,n.iter=100,n.burnin=90,n.chains=2,DIC=F)
    )
    return(model)
}
...
result<-chains$phylogenetic_attraction %>% filter(parameter %in% c("omega","gamma","lambda_cov")) %>% select(-par) %>% spread(parameter,value) %>% group_by(Draw,chain) %>% do(tauC(omega=.$omega,gamma=.$gamma,I=I,D=D,lambda_cov=.$lambda_cov)) %>% group_by(Var1,Var2) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
...
result<-chains$trait_attraction %>% filter(parameter %in% c("omega","gamma","lambda_cov")) %>% select(-par) %>% spread(parameter,value) %>% group_by(Draw,chain) %>% do(tauC(omega=.$omega,gamma=.$gamma,I=I,D=Dtraits,lambda_cov=.$lambda_cov)) %>% group_by(Var1,Var2) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
...
result<-chains$trait_repulsion %>% filter(parameter %in% c("omega","gamma","lambda_cov")) %>% select(-par) %>% spread(parameter,value) %>% group_by(Draw,chain) %>% do(tauC(omega=.$omega,gamma=.$gamma,I=I,D=Dtraits,lambda_cov=.$lambda_cov)) %>% group_by(Var1,Var2) %>% summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
...
## E: The effect of autocorrelation on mean flowering occurrence
...
```{r}
allchains %>% filter(parameter %in% "e") %>% group_by(Model,jPlant,Month) %>% mutate(value=inv.logit(value)) %>%  summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95)) %>% inner_join(jagsIndexPlants) %>% ggplot(.,aes(y=mean,x=Model,col=as.factor(Month))) + geom_pointrange(aes(ymin=lower,ymax=upper)) + facet_wrap(~Plant) + labs(y="Effect of Association on Flowering",x="",col="Site") + coord_flip()
```
...
```{r}
indat %>% filter(Plant=="Glossoloma purpureum") %>% ggplot(.) + geom_jitter(aes(x=as.factor(Month),y=n),height=0.01)

allchains %>% filter(parameter %in% "Ynew",Plant=="Glossoloma purpureum") %>% ggplot(.,aes(fill=as.factor(Month))) + geom_density(aes(x=value)) + theme_bw() + labs(x="Ynew")

allchains %>% filter(parameter %in% "alpha",Plant=="Glossoloma purpureum") %>% ggplot() + geom_density(aes(x=inv.logit(value))) + theme_bw() + labs(x="Alpha")

allchains %>% filter(parameter %in% "e",jPlant==13) %>% ggplot() + geom_density(aes(x=inv.logit(value),fill=Model),alpha=0.5) + theme_bw() + labs(x="e") + facet_wrap(~Month)
...
## Alpha: Species occurrence probability

```{r}
allchains %>% filter(parameter %in% "alpha") %>% ggplot(.,aes(x=inv.logit(value),fill=Model)) + geom_density(alpha=0.5)  + labs(x="alpha") + facet_wrap(~Plant,scale="free")
```
...
```{r}
allchains %>% filter(parameter %in% "pred_error") %>%  group_by(Model) %>% ggplot(.,aes(x=value,fill=Model)) + geom_density(alpha=0.5)  + labs(x="Prediction Error") + theme_bw() + facet_wrap(~Plant,scales="free")
```
...
allchains %>% filter(parameter %in% "Ynew") %>% mutate(value=value) %>%  group_by(Model,Plant) %>% summarize(n=sum(value)/n())  %>% arrange(Plant,Model) %>% kable() %>% kable_styling()
